...
将 `.dsh` 目录复制到 `$DSH_HOME/.agent-presets/harness-specs/`：
...
重启 DeepSeek Harness 后，在会话预设列表选择 `Hermes Harness 规格文档专家`。

### 方式二：仅安装技能（skill）

将 SKILL.md 复制到 `$DSH_HOME/skills/dsh-harness-specs/`：

```powershell
$dst = Join-Path $env:DSH_HOME "skills\dsh-harness-specs"
New-Item -ItemType Directory -Force -Path $dst | Out-Null
Copy-Item ".\dsh\skills\dsh-harness-specs\SKILL.md" $dst -Force
```
